...
class CASTModel(BaseModel):
    ...
    def adv_forward(self, adv, calc_mean_std, artist_name):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        style_name = Path(self.input['B_paths'][0]).stem
        if adv == True:
            import ipdb
            ipdb.set_trace()
            self.netAE(self.real_A, self.real_B, adv = adv, calc_mean_std = calc_mean_std, style_name = style_name, artist_name = artist_name)
        else:
            self.real_A_feat = self.netAE(self.real_A, self.real_B, calc_mean_std = calc_mean_std, style_name = style_name)  # G_A(A)
        # self.fake_B = self.netDec_B(self.real_A_feat)
    
    def adv_end_to_end(self, content_images, dir_name):
        ...
        style_name = Path(self.input['B_paths'][0]).stem
        x_adv = self.real_B.detach() + 0.001 * torch.randn(self.real_B.shape).cuda().detach()
        clean_output = self.netDec_B(self.netAE(self.real_A, self.real_B)) 
        for content in content_images: 
            content = content.unsqueeze(0)
            for _step in range(10):
                x_adv.requires_grad_()
                with torch.enable_grad():
                    output = self.netDec_B(self.netAE(content, x_adv))  
                loss = criterion(clean_output, output)
                # import ipdb
                # ipdb.set_trace()    
                grad = torch.autograd.grad(loss, [x_adv])[0]
                x_adv = x_adv.detach() + alpha * torch.sign(grad.detach())

                x_adv = torch.min(torch.max(x_adv, self.real_B - epsilon), self.real_B + epsilon)
